...
training video dayLane.webm):
  ...
  - Main output changed to black background + white Canny edges, cropped to
    the road ROI (matches img/canny2.jpg style in the source repo).
...
MAX_LINES_FOR_CLUSTERING = 4000  # safety cap: busy/textured scenes can generate
                                  # tens of thousands of raw Hough lines, and
                                  # DBSCAN on that many points can exhaust memory
                                  # and get the process killed. Debug-overlay only.

MORPH_KERNEL = np.ones((3, 3), np.uint8)  # cleans salt-and-pepper noise from mask
MIN_BLOB_AREA = 40  # px; drops leftover small dot clusters after Canny
...
def get_cluster_lines(lines, eps=DBSCAN_EPS, min_samples=DBSCAN_MIN_SAMPLES):
    ...
    if lines.shape[0] > MAX_LINES_FOR_CLUSTERING:
        idx = np.random.choice(lines.shape[0], MAX_LINES_FOR_CLUSTERING, replace=False)
        lines = lines[idx]
...
def remove_small_blobs(binary_img, min_area=MIN_BLOB_AREA):
    """Drops connected components smaller than min_area (removes leftover dot noise)."""
...
def process_image(img, debug_name=None, debug=False):
    ...
    mask_clean = cv2.morphologyEx(mask, cv2.MORPH_OPEN, MORPH_KERNEL)  # kills 1-2px speckle noise
    filtered_clean = cv2.bitwise_and(bilateral, bilateral, mask=mask_clean)
    canny = cv2.Canny(filtered_clean, 100, 255)

    roi_pts = get_roi_points(h, w)
    cropped_canny = region_of_interest(canny, roi_pts)
    cropped_canny = remove_small_blobs(cropped_canny)  # drops remaining dot clusters

    # Main output: black background, white Canny edges, cropped to the road ROI
    # (matches img/canny2.jpg style in the source repo — thin edge outlines,
    # not filled color-mask blobs).
    lanes_only = cv2.cvtColor(cropped_canny, cv2.COLOR_GRAY2BGR)
